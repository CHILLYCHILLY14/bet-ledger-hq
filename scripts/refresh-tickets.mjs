...
const now=Date.now(),stamp=new Date(now).toISOString(),base='https://chillychilly14.github.io/bet-ledger-hq/';
...
async function get(url){const response=await fetch(url,{signal:AbortSignal.timeout(15000),headers:{'User-Agent':'KEVBOT-public-ticket-archive/1.0'}});if(!response.ok)throw Error('HTTP '+response.status);return response.json();}
